...
async function run() {
  ...
  const template = await Template.findOne({ status: 'draft', name: 'rs_payment_reminder_v1' });
  if (!template) {
  ...
  // Exact payload logic from my fix
  let components = JSON.parse(JSON.stringify(template.components || []));
  const payload = {
    name: template.name + '_' + Date.now(), // append timestamp to avoid name collision for test
    language: 'en_US',
