...
import { normalizeTags, syncTagsFromConversation } from '../services/leadTagSyncService.js';
...
const isAgentRequest = (req) => req.user?.role === 'agent';

const buildConversationIdentityFilter = (conversationId, accountId) => {
  const normalizedConversationId = String(conversationId || '');
  const filter = {
    accountId,
    $or: [{ conversationId: normalizedConversationId }]
  };
...
const ensureAgentCanOpenConversation = async (req, res, conversationId) => {
  ...
  if (!userId) {
    res.status(403).json({
      success: false,
      message: 'Agent user id missing. Please login again.',
      error: 'AGENT_USER_ID_MISSING'
    });
    return false;
  }

  const conversation = await Conversation.findOne(
    buildConversationIdentityFilter(conversationId, req.account.accountId)
  ).select('assignedAgentId conversationId').lean();
  ...
  if (String(conversation.assignedAgentId || '') !== String(userId)) {
    res.status(403).json({
      success: false,
...
router.get('/', async (req, res) => {
  try {
    ...
    const forceAssignedToMe = isAgentRequest(req);
    if (forceAssignedToMe && !req.user.userId) {
    ...
    const filters = {
      ...
      assignedToMe: forceAssignedToMe || assignedToMe === 'true',
      agentId: (forceAssignedToMe || assignedToMe === 'true') ? req.user.userId : null,
...
router.get('/:conversationId', async (req, res) => {
  try {
    ...
    if (!(await ensureAgentCanOpenConversation(req, res, conversationId))) return;
...
router.patch('/:conversationId', async (req, res) => {
  try {
    ...
    if (tags && Array.isArray(tags)) updates.tags = normalizeTags(tags);
    ...
    if (updates.tags) {
      await syncTagsFromConversation(accountId, conversationId, updates.tags);
    }
...
router.post('/:conversationId/assign', async (req, res) => {
  try {
    const { conversationId } = req.params;
    const requestedAgentId = req.body?.agentId || null;
    const projectId = req.body?.projectId || req.query?.projectId || null;
    const accountId = req.account.accountId;
    let agentId = req.user?.userId || null;
    let assignmentAgentId = req.user?.agentMongoId || null;

    if (requestedAgentId) {
      const agentFilters = [
        { agentId: String(requestedAgentId) }
      ];
      if (mongoose.Types.ObjectId.isValid(String(requestedAgentId))) {
        agentFilters.push({ _id: requestedAgentId }, { userId: requestedAgentId });
      }
      ...
      const agent = await Agent.findOne({
        ...
        ...(projectId ? { projectId } : {}),
        $or: agentFilters
      }).select('_id agentId userId name email projectId');

      if (!agent?.userId) {
        return res.status(404).json({
          success: false,
          message: 'Active agent not found for this project',
          error: 'AGENT_NOT_FOUND'
        });
      }

      agentId = agent.userId;
      assignmentAgentId = agent._id;
    }
...
router.post('/:conversationId/close', async (req, res) => {
  try {
    ...
    const accountId = req.account.accountId;
    const closedByAgentId = req.user.userId;
...
router.post('/:conversationId/reopen', async (req, res) => {
  try {
    const { conversationId } = req.params;
    const accountId = req.account.accountId;
    const reopenedByAgentId = req.user.userId;
...
router.post('/:conversationId/notes', async (req, res) => {
  try {
    ...
    const accountId = req.account.accountId;
    const createdByAgentId = req.user.userId;
