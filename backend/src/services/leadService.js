...
const syncLeadToInternalCRM = async (lead) => {
  ...
  try {
    ...
    await Contact.findOneAndUpdate(
      { accountId: ACCOUNT_ID, whatsappNumber: phone },
      {
        $set: {
    ...
    logger.error('❌ Error syncing lead to internal CRM:', error.message);
  }
};
/**
...
export const captureLeadFromConversation = async (accountId, conversationId) => {
  try {
    ...
    if (existingLead) {
      ...
      await syncLeadToInternalCRM(existingLead);
      await upsertEducationEnquiry({
        accountId,
        projectId: existingLead.projectId || conversation.projectId || null,
        phone: contact.whatsappNumber || contact.phoneNumber || '',
        name: contact.name,
        email: contact.email || '',
        notes: existingLead.sourceMessage,
        source: 'live_chat_auto_capture',
      });
      return existingLead;
    } else {
      // Create new lead
      ...
      await lead.save();
      await syncLeadToInternalCRM(lead);
      await upsertEducationEnquiry({
        accountId,
...
/**
 * Auto-capture lead when they open/read a campaign
 */
export const captureLeadFromCampaignEngagement = async (accountId, conversationId, campaignId, action = 'opened') => {
  try {
    ...
    // Create new lead since they opened the campaign
    const leadData = {
      accountId,
      projectId: conversation.projectId || null,
      conversationId,
      ...
      email: contact.email || '',
      phone: contact.whatsappNumber || contact.phone || '',
      company: contact.company || '',
      intent: 'other',
      keywords: [],
      messageCount: 0,
      firstMessage: new Date(),
      lastMessage: new Date(),
      sourceMessage: `[Campaign ${action}]`,
      status: 'new'
    };

    const lead = new Lead(leadData);
    lead.score = 50;
    lead.scoreBreakdown = { engagement: 10, intent: 20, recency: 20, completion: 0 };
    
    await lead.save();
    
    // Update contact status to 'new' since they are now a lead
    await Contact.findByIdAndUpdate(contact._id, { $set: { leadStatus: 'new' } });
    
    await syncLeadToInternalCRM(lead);
    return lead;
  } catch (error) {
    logger.error('❌ Campaign Lead capture error:', error.message);
    return null;
  }
};
  ...
  try {
    ...
    if (filters.search) {
      query.$or = [
        ...
        { sourceMessage: { $regex: filters.search, $options: 'i' } }
      ];
    }
...
export default {
  ...
  detectIntent,
  extractKeywords,
  captureLeadFromCampaignEngagement
};
