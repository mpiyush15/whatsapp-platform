...
class CreditLedgerService {
  ...
  async postLedgerEntry({
    ...
    if (!account) {
      if (isDebit) {
        // Atomic floor constraint: only increment if the resulting balance would remain >= 0
        account = await Account.findOneAndUpdate(
          { accountId, creditBalance: { $gte: Math.abs(signedAmount) } },
          { $inc: { creditBalance: signedAmount } },
          { new: true }
        );

        if (!account) {
          // It could be null because the account doesn't exist, OR because the balance was too low.
          // Let's check if the account exists at all to throw the correct error.
    ...
    // --- LOW CREDIT ALERT TRIGGER ---
    try {
      if (isDebit && entryType === 'usage_debit') {
        const CreditPackSettings = await import('../models/CreditPackSettings.js').then(m => m.default);
        const settings = await CreditPackSettings.findOne().select('lowCreditWarningThreshold');
        const threshold = Number(settings?.lowCreditWarningThreshold ?? 200);

        if (balanceBefore > threshold && balanceAfter <= threshold) {
          const { emailService } = await import('./emailService.js');
          await emailService.sendLowCreditWarningEmail(
            account.email,
            account.name || 'Customer',
            balanceAfter,
            threshold
          ).catch(err => logger.error('⚠️ Low credit email error:', err.message));
          
          logger.info(`📧 Low credit warning email sent to ${account.email} (Balance: ${balanceAfter}, Threshold: ${threshold})`);
        }
      }
    } catch (alertErr) {
      logger.error('⚠️ Failed to check/send low credit alert:', alertErr.message);
    }
