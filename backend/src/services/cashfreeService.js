...
export const cashfreeService = {
  ...
  createOrder: async (orderData) => {
    try {
      ...
      // On VPS, FRONTEND_URL and BACKEND_URL from .env take precedence. 
      // If we are in production and it's missing https, we enforce it to avoid Cashfree errors.
      if (process.env.NODE_ENV === 'production') {
        if (frontendUrl.startsWith('http://localhost') || !frontendUrl.startsWith('http')) {
          frontendUrl = 'https://app.replysys.com'; // Safe production fallback
        } else if (!frontendUrl.startsWith('https://')) {
          frontendUrl = frontendUrl.replace('http://', 'https://');
        }
        
        if (backendUrl.startsWith('http://localhost') || !backendUrl.startsWith('http')) {
          backendUrl = 'https://app.replysys.com'; // Safe production fallback
  ...
  syncPaymentsFromCashfree: async () => {
    try {
      ...
      // Step 1: Get pending/processing order IDs from our Payment DB (sorted newest first)
      logger.info('📚 Fetching pending order IDs from local database...');
      ...
      for (const order of orders) {
        try {
          ...
          if (order.payments && Array.isArray(order.payments)) {
            hasSuccessfulPayment = order.payments.some(p => p.payment_status === 'SUCCESS');
          }

          // Skip if neither the order is PAID nor it has a SUCCESS payment
          if (order.order_status !== 'PAID' && !hasSuccessfulPayment) {
          ...
          // ✅ TRIGGER LIFECYCLE FOR COMPLETED PAYMENTS TO UPGRADE ACCOUNT!
          if (paymentStatus === 'completed') {
            try {
              const billingLifecycleService = (await import('./billingLifecycleService.js')).default;
              await billingLifecycleService.processPaymentLifecycle({
                body: {
                  orderId: order.order_id,
                  orderStatus: order.order_status,
              ...
              logger.info(`✅ Lifecycle completed for ${order.order_id}`);
            } catch (lifecycleErr) {
              logger.error(`❌ Lifecycle trigger failed for ${order.order_id}:`, lifecycleErr);
            }
          }
