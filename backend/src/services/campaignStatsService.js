...
export async function refreshCampaignStatsFromMessages(campaignId, accountId) {
  ...
  if (campaign.sentPhones && campaign.sentPhones.length > 0) {
    const phoneVariations = [];
    campaign.sentPhones.forEach(p => {
      const pStr = String(p);
    ...
    try {
      ...
      for (const stat of contactStats) {
        const status = String(stat._id || '').toLowerCase();
        if (status === 'qualified') {
          totalQualified += stat.count;
        } else if (status === 'won') {
          totalWon += stat.count;
          estimatedRevenue += stat.value || 0;
        } else if (status === 'lost') {
          totalLost += stat.count;
        }
      }
    } catch (err) {
      logger.error(`Error aggregating contact stats for campaign ${campaignIdStr}: ${err.message}`);
    }
  }
  ...
  const conversionRate = (deliveredOrReadCount > 0 ? (totalWon / deliveredOrReadCount) * 100 : 0);
  ...
  await campaign.save();
  logger.info(
    `[Campaign:${campaignIdStr}] stats refreshed: sent=${sentCount} read=${readCount} replied=${totalReplied} qualified=${totalQualified} won=${totalWon} lost=${totalLost} rev=${estimatedRevenue}`
  );
  return campaign;
}
...
async function getAudiencePhonesFromCampaign(campaign, accountId) {
  if (campaign?.audience?.type === 'all') {
    const contacts = await Contact.find({
      ...
      source: { $nin: AUTO_EDUCATION_CONTACT_SOURCES },
      isOptedIn: { $ne: false },
    })
