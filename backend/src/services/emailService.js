...
export const emailService = {
  ...
  // Send project-scoped login details for a support agent
  sendAgentLoginEmail: async ({ to, name, accountName, loginUrl, temporaryPassword, projectId }) => {
    try {
      logger.info('📧 [EMAIL SERVICE] Sending agent login details...');
      logger.info('  To:', to);
      logger.info('  Agent:', name);
      logger.info('  Project:', projectId || 'default');
      ...
      await sendViaZepto(
        to,
        `Your ${accountName || 'Replysys'} live chat login`,
        `
          <!DOCTYPE html>
          <html>
          <head>
            <style>
              body { font-family: Arial, sans-serif; color: #333; }
              .container { max-width: 600px; margin: 0 auto; padding: 20px; }
              .header { background: #111827; color: white; padding: 28px; border-radius: 8px; }
              .content { padding: 28px; background: #f9fafb; border-radius: 8px; margin-top: 18px; }
              .button { background: #128c7e; color: white; padding: 13px 24px; text-decoration: none; border-radius: 4px; display: inline-block; font-weight: bold; }
              .password { letter-spacing: 8px; font-size: 28px; font-weight: bold; background: white; border: 1px solid #e5e7eb; padding: 16px; text-align: center; border-radius: 8px; }
          ...
          <body>
            <div class="container">
              <div class="header">
                <h2>Your live chat login is ready</h2>
                <p>${accountName || 'Replysys'} added you as a support agent.</p>
              </div>
              <div class="content">
                <p>Hello ${name},</p>
                <p>Use your email address and this 6 digit password to open your assigned project inbox.</p>
                <p class="password">${temporaryPassword}</p>
                <p style="text-align:center; margin: 24px 0;">
                  <a href="${loginUrl}" class="button">Open Live Chat</a>
                </p>
                <p class="muted">Login email: <strong>${to}</strong></p>
                <p class="muted">If the button does not open, copy this URL into your browser:</p>
                <p class="muted" style="word-break: break-all;">${loginUrl}</p>
              </div>
              <div class="footer">
                <p>© ${new Date().getFullYear()} Replysys. This is an automated email.</p>
              </div>
            </div>
          </body>
          </html>
      ...
      logger.info('✅ Agent login email sent to', to);
      return { success: true };
    } catch (error) {
      logger.error('❌ Agent login email failed:', error.message);
      return { success: false, error: error.message };
    }
  },
  ...
  sendAgentInvitationEmail: async (agentEmail, agentName, invitationToken, accountName) => {
    try {
      ...
      const invitationUrl = `${getPrimaryFrontendUrl()}/agent-onboarding?token=${invitationToken}&email=${encodeURIComponent(agentEmail)}`;
