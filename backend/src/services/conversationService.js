 ...
 */
export const getConversationDetail = async (conversationId, accountId, projectId = null) => {
  const conversation = await conversationRepository.findByIdAndAccount(conversationId, accountId, projectId);
 ...
 */
export const assignConversation = async (conversationId, assigneeUserId, accountId, reason = 'manual', assignmentAgentId = assigneeUserId, projectId = null) => {
  const conversation = await conversationRepository.assignToAgent(
    conversationId,
    accountId,
    assigneeUserId,
    reason,
  ...
  // Audit timeline uses Conversation._id, while live chat routes use conversationId string.
  try {
    await ActivityTimeline.create({
      ...
      relatedAgentId: assignmentAgentId,
      details: new Map([
    ...
    logger.warn('⚠️ Assignment saved but timeline audit failed', {
      conversationId,
 ...
 * Close conversation
 */
export const closeConversation = async (conversationId, accountId, reason = 'manual', closedByAgentId = null, projectId = null) => {
 ...
 * Reopen conversation
 */
export const reopenConversation = async (conversationId, accountId, reopenedByAgentId = null, projectId = null) => {
  const conversation = await conversationRepository.updateStatus(conversationId, accountId, 'open', {}, projectId);
 ...
 */
export const addTagToConversation = async (conversationId, tagName, accountId, projectId = null) => {
  // Verify tag exists
  ...
  // Add tag to conversation
  const conversation = await conversationRepository.addTag(conversationId, accountId, tagName, projectId);
 ...
 */
export const removeTagFromConversation = async (conversationId, tagName, accountId, projectId = null) => {
  const conversation = await conversationRepository.removeTag(conversationId, accountId, tagName, projectId);
 ...
 */
export const updateConversationMetrics = async (conversationId, accountId, projectId = null) => {
  // Count messages
  ...
  const conversation = await conversationRepository.findLeanByIdAndAccount(conversationId, accountId, projectId);
 ...
 */
export const markConversationAsRead = async (conversationId, agentId, accountId, projectId = null) => {
  const conversation = await conversationRepository.markAsRead(conversationId, accountId, agentId, projectId);
