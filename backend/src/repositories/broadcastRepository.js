...
class BroadcastRepository {
  ...
  /**
   * Atomically increment stats (used by parallel BullMQ workers).
   *
   * @param {string} broadcastId
   * @param {{ sent?: number, failed?: number, pending?: number, inProgress?: number }} incs
   * @returns {Promise<void>}
   */
  async incrementProgress(broadcastId, incs) {
    const $inc = {};
    if (incs.sent !== undefined) $inc['stats.sent'] = incs.sent;
    if (incs.failed !== undefined) $inc['stats.failed'] = incs.failed;
    if (incs.pending !== undefined) $inc['stats.pending'] = incs.pending;
    if (incs.inProgress !== undefined) $inc['stats.inProgress'] = incs.inProgress;

    await Broadcast.findByIdAndUpdate(broadcastId, { $inc });
  }
