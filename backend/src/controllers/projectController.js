...
export async function createProject(req, res) {
  try {
    ...
    // Enforce WhatsApp number global uniqueness (No Crossing Rule)
    if (whatsappPhoneNumberId) {
      const existingProject = await Project.findOne({ whatsappPhoneNumberId });
      if (existingProject) {
        return res.status(409).json({
          success: false,
          error: 'This WhatsApp number is already connected to another workspace. Please use a fresh number.'
        });
      }
    }
    ...
    const redirectPath =
      presetResult?.defaultHomePath === 'healthcare'
        ...
        : presetResult?.defaultHomePath === 'pathology'
          ...
          : presetResult?.defaultHomePath === 'education/enquiries'
            ? `/projects/${projectId}/education/enquiries`
          : `/projects/${projectId}`;
...
export async function updateProject(req, res) {
  try {
    ...
    // Enforce WhatsApp number global uniqueness (No Crossing Rule)
    if (updates.whatsappPhoneNumberId) {
      const existingProject = await Project.findOne({
        whatsappPhoneNumberId: updates.whatsappPhoneNumberId,
...
export async function deleteProject(req, res) {
  try {
    ...
    // Hard delete
    await Project.findOneAndDelete({ projectId, accountId });
