...
import platformWhatsAppService from '../services/platformWhatsAppService.js';
  ...
  try {
    ...
    if (user) {
      if (user.role === 'agent' || user.role === 'manager') {
        ...
        }).select('_id agentId projectId role name email');

        if (agentProfile) {
          tokenUser.agentId = agentProfile.agentId;
          tokenUser.agentMongoId = String(agentProfile._id);
          tokenUser.agentRole = agentProfile.role;
          tokenUser.agentProjectId = agentProfile.projectId || null;
        }
      }
...
export const signup = async (req, res) => {
  try {
    ...
    try {
      await emailService.sendWelcomeEmail(email, name).catch(err => logger.error('⚠️ Failed to send welcome email:', err.message));
      
      // Send Internal WhatsApp Welcome Message (Using the ReplySys Internal Engine)
      if (phone) {
        await platformWhatsAppService.sendWelcome(phone, name, planName).catch(err => logger.error('⚠️ Failed to send welcome whatsapp:', err.message));
      }
    } catch (emailErr) {
      logger.error('⚠️ Welcome Message error:', emailErr.message);
    }
