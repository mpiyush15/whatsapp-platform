...
import {
  buildEducationPresetRule,
  ensureEducationPresetRules,
  getEducationPresetDefinitions,
} from '../services/educationChatbotPresetService.js';
...
const INTERNAL_EDUCATION_PRESET_PROJECT_IDS = new Set(['proj_wa_1780249392038']);
...
const leadSafeResponses = (value) => Object.fromEntries(
  Object.entries(normalizeResponseObject(value))
...
const hasEducationContext = async (accountId, projectId) => {
  const [course, batch, enquiry] = await Promise.all([
    Course.exists({ accountId, projectId }),
    Batch.exists({ accountId, projectId }),
    Enquiry.exists({ accountId, projectId })
  ]);
  return Boolean(course || batch || enquiry);
};

const ensureEducationPresetsForProject = async (accountId, projectId) => {
  if (await hasEducationContext(accountId, projectId)) {
    await ensureEducationPresetRules({ KeywordRule, accountId, projectId });
  }
};
...
export const getChatbots = async (req, res) => {
  try {
    ...
    if (projectId) await ensureEducationPresetsForProject(accountId, projectId);
...
export const getEducationChatbotPresets = async (req, res) => {
  try {
    const accountId = getAccountId(req);
    const projectId = getProjectId(req);

    if (accountId && projectId) await ensureEducationPresetsForProject(accountId, projectId);

    return sendSuccess(res, { presets: getEducationPresetDefinitions() }, 'Education chatbot presets');
  } catch (error) {
    return handleControllerError(res, error, 'getEducationChatbotPresets');
  }
};

export const installEducationChatbotPreset = async (req, res) => {
  try {
    const accountId = getAccountId(req);
    const projectId = getProjectId(req);

    if (!accountId || !projectId) {
      return sendValidationError(res, 'Project context required');
    }

    const preset = normalizeString(req.body?.preset || 'education_enquiry');
    const fields = normalizeArray(req.body?.fields);
    const isActive = req.body?.isActive !== false;
    const baseRule = buildEducationPresetRule({ projectId, preset, fields, isActive });
    ...
    const rule = await KeywordRule.findOneAndUpdate(
      {
        ...
        projectId,
        name: baseRule.name,
        'replyContent.workflow.0.id': 'welcome',
      },
      {
        $set: {
          ...baseRule,
    ...
    return sendSuccess(res, serializeRule(rule), 'Education chatbot preset installed', 201);
  } catch (error) {
    return handleControllerError(res, error, 'installEducationChatbotPreset');
  }
};
...
export const getChatbotLeads = async (req, res) => {
  try {
    ...
    const sessionLeads = sessions
      ...
      .map((session) => {
        const responses = leadSafeResponses(session.responses);
...
export const updateLead = async (req, res) => {
  try {
    ...
    const projectId = getProjectId(req);
    const query = { _id: leadId, accountId };
    if (projectId) query.projectId = projectId;

    const lead = await ChatbotLead.findOne(query);
    if (!lead) {
...
export const deleteLead = async (req, res) => {
  try {
    ...
    const deleted = await ChatbotLead.findOneAndDelete(query);
    if (!deleted) {
      return sendNotFound(res, 'Lead');
    }
