...
export const getConversation = async (req, res) => {
  try {
    ...
    const projectId = req.query?.projectId || req.projectId || null;
    const query = { conversationId, accountId };
    if (projectId) query.projectId = projectId;

    const conversation = await Conversation.findOne(query).populate('assignedAgentId', 'name email status');
    ...
    const msgQuery = { conversationId, accountId };
    if (projectId) msgQuery.projectId = projectId;
...
export const sendMessage = async (req, res, io) => {
  try {
    ...
    const projectId = req.query?.projectId || req.body?.projectId || req.projectId || null;
    const query = { conversationId, accountId };
    if (projectId) query.projectId = projectId;
    ...
    conversation.hasInboundMessage = true; // Mark as active since agent engaged
