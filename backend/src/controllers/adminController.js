...
import { META_API } from '../constants/whatsapp.js';
...
export const adminConnectWhatsApp = async (req, res) => {
  try {
    const { accountId } = req.params;
    const { waba_id, phone_number_id, custom_access_token } = req.body;

    const systemToken = custom_access_token || process.env.META_SYSTEM_TOKEN;
    if (!systemToken) {
      return res.status(400).json({ success: false, message: 'No system token available' });
    }
    ...
    const phoneDetailsResponse = await axios.get(phoneDetailsUrl, {
      params: { fields: 'display_phone_number,quality_rating,verified_name', access_token: systemToken }
    });
    ...
    await Account.findOneAndUpdate(
      { accountId },
      {
        ...
        whatsappConfig: { wabaId: waba_id, connectedAt: new Date(), source: 'ADMIN_MANUAL' }
      }
    );
    ...
    return res.json({ success: true, message: 'WhatsApp connected successfully via Admin' });
  } catch (error) {
    logger.error('Admin WhatsApp Connect Error:', error?.response?.data || error);
