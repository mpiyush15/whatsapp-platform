...
export const verifyPayment = async (req, res) => {
  try {
    ...
    if (payment.lifecycleState === 'completed' || payment.status === 'completed') {
      ...
      const creditsGranted = paymentOrderType === 'credits'
        ? Number(payment?.metadata?.credits || payment?.pricingSnapshot?.credits || 0)
        : 0;
      return sendSuccess(res, {
        processed: true,
        ...
        creditsGranted,
        creditBalance: paymentOrderType === 'credits' ? Number(account?.creditBalance || 0) : undefined,
...
export const buyCredits = async (req, res) => {
  try {
    ...
    if (packId) {
      ...
      price = pack.price;
      if (Number(credits) <= 0 || Number(price) <= 0) {
        return sendValidationError(res, 'Credit pack must have credits and price above 0');
      }
      packMetadata = {
        packId: pack.packId,
        name: pack.name,
      ...
      if (Number(credits) <= 0 || Number(price) <= 0) {
        return sendValidationError(res, 'Custom amount must create at least 1 credit');
      }
    ...
    if (!orderResult.success) {
      ...
      return sendError(res, orderResult.error || 'Failed to initiate payment', 500);
    }
