...
export const connectWhatsApp = async (req, res) => {
  ...
  try {
    ...
    // Step 1.5: Enforce No-Crossing Rule if projectId is provided
    if (projectId) {
      ...
      if (existingProject) {
        return sendValidationError(res, 'This WhatsApp number is already connected to another workspace. Please use a fresh number.');
      }
    }
    ...
    // Step 3.5: Save to Project model
    if (projectId) {
      ...
      logger.info(`✅ Project ${projectId} updated with WhatsApp credentials`);
    }
...
export const getConnectedPhones = async (req, res) => {
  try {
    ...
    let phones = await PhoneNumber.find({ accountId, isActive: true });

    if (projectId) {
      const project = await Project.findOne({ projectId, accountId });
      if (project && project.whatsappPhoneNumberId) {
        phones = phones.filter(p => p.phoneNumberId === project.whatsappPhoneNumberId);
      } else {
        phones = [];
      }
    }
