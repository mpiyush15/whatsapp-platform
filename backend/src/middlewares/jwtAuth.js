...
export const requireJWT = async (req, res, next) => {
  try {
    ...
    let raw = typeof authHeader === 'string' ? authHeader.replace(/^Bearer\s+/i, '').trim() : '';
    if (!raw && req.query.auth_token) {
      raw = req.query.auth_token;
    }
    ...
    req.user = {
      ...
      agentMongoId: decoded.agentMongoId,
      agentProjectId: decoded.agentProjectId,
    };
...
export const generateToken = (user) => {
  ...
  return jwt.sign(
      ...
      agentMongoId: user.agentMongoId,
      agentProjectId: user.agentProjectId
    },
    JWT_SECRET,
...
export const requireSuperAdmin = (req, res, next) => {
  if (req.account?.type !== 'internal' || req.user?.role !== 'superadmin') {
