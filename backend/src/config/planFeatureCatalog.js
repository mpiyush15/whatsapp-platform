...
export const LIMIT_KEYS = [
  ...
  { key: 'chatbots', label: 'Chatbots created', productLine: 'whatsapp', unit: 'count' },
...
export function buildFeatureMatrix(plans, productLine, customCatalog = null) {
  const catalog = customCatalog || catalogForProductLine(productLine);
  const rows = [
  ...
  const planCells = plans.map((plan) => {
    ...
    for (const row of rows) {
      ...
      if (row.kind === 'limit') {
        ...
        if (typeof ent[row.key] === 'string') {
          cells[cellKey] = ent[row.key];
        } else {
