...
import { buildHealthcareBookingGraph, buildHealthcareRegistrationGraph, buildHealthcareSlotCheckerGraph } from "./healthcarePresets"
    ...
    actions: [
      {
        ...
        description: 'Saves collected WhatsApp responses into the education enquiries table',
        configFields: [
          { key: 'nameVar', label: 'Student Name Variable', placeholder: 'e.g. {{name}}' },
          { key: 'emailVar', label: 'Email Variable', placeholder: 'e.g. {{email}}' },
          { key: 'courseId', label: 'Course Variable', placeholder: 'Use {{course__id}} from dynamic courses' },
          { key: 'batchId', label: 'Batch Variable', placeholder: 'Optional: {{batch__id}} from dynamic batches' },
          { key: 'feesVar', label: 'Fees Variable (₹)', placeholder: 'Optional: {{fees}}' },
          { key: 'notesVar', label: 'Notes Variable', placeholder: 'Optional: {{notes}}' },
        ],
      },
    ],
  },
...
const inferVariableName = (text = "", fallback = "selection") => {
  ...
  if (/\bservice\b|\bhelp\b|\boption\b/.test(lower)) return "service"
...
function ButtonsNode({ data }: NodeProps<FlowNodeData>) {
  ...
  return (
    <div style={baseNodeStyle} className="border-amber-200 bg-amber-50 p-4">
      ...
      <div className="mt-2 text-[11px] text-amber-700">Save as: {data.saveAs || inferVariableName(data.text, "selection")}</div>
...
function ListNode({ data }: NodeProps<FlowNodeData>) {
  ...
  const isDynamic = Boolean(data.dynamicList)
  return (
    <div style={baseNodeStyle} className="border-violet-200 bg-violet-50 p-4">
      ...
      <div className="mt-2 text-[11px] text-violet-700">Save as: {data.saveAs || inferVariableName(data.text, "selection")}</div>
      {isDynamic ? (
        <div className="mt-2 rounded-md bg-white px-2 py-1 text-[11px] font-medium text-violet-700">
          Dynamic: {data.dynamicList}
        </div>
      ) : null}
      <div className="mt-3 space-y-1">
        {items.length === 0 ? <div className="text-[11px] text-slate-500">{isDynamic ? 'Options load from project data at runtime' : 'No list items yet'}</div> : null}
...
const buildEducationEnquiryGraph = () => {
  const startId = makeId("start")
  const welcomeId = makeId("welcome")
  const nameId = makeId("name")
  const emailId = makeId("email")
  const courseId = makeId("course")
  const detailsId = makeId("details")
  const saveId = makeId("save-enquiry")
  const endId = makeId("end")
  ...
  const nodes: Node<FlowNodeData>[] = [
    ...
    },
    {
      ...
      type: "message",
      position: { x: 80, y: 220 },
      data: { label: "Welcome", text: "Welcome to our academy. I can help you choose the right course.", delay: 0 },
    },
    {
      id: nameId,
      type: "question",
      position: { x: 80, y: 380 },
      data: { label: "Ask Name", text: "Please share the student's full name.", saveAs: "name", delay: 0 },
    },
    {
      id: emailId,
      type: "question",
      position: { x: 80, y: 540 },
      data: { label: "Ask Email", text: "Thanks {{name}}. Please share an email address for admission updates.", saveAs: "email", delay: 0 },
    },
    {
      id: courseId,
      type: "list",
      ...
      data: {
        ...
        text: "{{name}}, please select the course you are interested in.",
        saveAs: "course",
    ...
    },
    {
      ...
      data: {
        label: "Ask Details",
        text: "Great choice: {{course}}. Share class/marks/location or any counselling question.",
        saveAs: "notes",
        delay: 0,
      },
    },
    ...
    },
    {
      ...
      data: { label: "Complete", text: "Thanks {{name}}. Your enquiry is saved. Our counsellor will contact you shortly." },
    },
  ]

  const chain = [startId, welcomeId, nameId, emailId, courseId, detailsId, saveId, endId]
...
function FlowBuilderStudioInner({ projectId, vertical = 'whatsapp' }: { projectId: string; vertical?: string }) {
  ...
  const createEducationEnquiryTemplate = () => {
    const template = buildEducationEnquiryGraph()
    setSelectedFlowId(null)
    setMeta({
      name: "Education course enquiry",
      description: "Welcomes the student, asks name/email, shows active courses, and saves the enquiry.",
      keywords: "hi, course, admission, fees",
      matchType: "contains",
      timeoutMinutes: 5,
      isActive: true,
    })
    ...
    setSelectedNodeId(null)
    setLeftTab("settings")
    setNotice("Education enquiry template loaded. Review keywords, then Save.")
    setError(null)
  }

  const createHealthcareRegistrationTemplate = () => {
    const template = buildHealthcareRegistrationGraph()
    setSelectedFlowId(null)
    setMeta({
      name: "Healthcare Registration",
      description: "Registers a new patient without leaking medical info.",
      keywords: "register, new patient, start",
      matchType: "contains",
      timeoutMinutes: 5,
      isActive: true,
    })
    ...
    setSelectedNodeId(null)
    setLeftTab("settings")
    setNotice("Healthcare Registration template loaded.")
    setError(null)
  }

  const createHealthcareSlotCheckerTemplate = () => {
    const template = buildHealthcareSlotCheckerGraph()
    setSelectedFlowId(null)
    setMeta({
      name: "Healthcare Slot Checker",
      description: "Allows patients to check doctor slots without booking.",
      keywords: "slots, schedule, availability",
      matchType: "contains",
      timeoutMinutes: 5,
      isActive: true,
    })
    ...
    setSelectedNodeId(null)
    setLeftTab("settings")
    setNotice("Healthcare Slot Checker template loaded.")
    setError(null)
  }

  const createHealthcareBookingTemplate = () => {
    const template = buildHealthcareBookingGraph()
    setSelectedFlowId(null)
    setMeta({
      name: "Healthcare Appointment Booking",
      description: "End-to-end booking flow syncing securely to the database.",
    ...
    setSelectedNodeId(null)
    setLeftTab("settings")
    setNotice("Healthcare Booking template loaded.")
    setError(null)
  }
    ...
    const newNode: Node<FlowNodeData> = {
      ...
      data:
        type === "start"
          ...
          : type === "buttons"
          ? { label: "Buttons", text: "Choose an option", saveAs: "selection", buttons: [{ id: makeId("button"), title: "Option 1", url: "" }] }
          : type === "list"
          ? { label: "List", text: "Pick from the list", saveAs: "selection", items: [{ id: makeId("item"), title: "Item 1", description: "" }] }
          : type === "condition"
          ? {
              label: "Condition",
              variable: "response",
  ...
  const savedVariables = useMemo(
    () =>
      nodes
        .map((node) => {
          if (!["question", "buttons", "list"].includes(String(node.type || ""))) return ""
          return String(node.data.saveAs || inferVariableName(node.data.text, String(node.type || "selection"))).trim()
        })
        .filter(Boolean),
    [nodes]
  )
  ...
  return (
    <div className="flex h-[calc(100vh-56px)] min-h-0 flex-col bg-slate-50">
      <div className="flex min-h-0 flex-1 flex-col xl:flex-row">
        {/* Left Sidebar */}
        <div className="flex min-h-[420px] w-full flex-col border-r border-slate-200 bg-white xl:min-h-0 xl:w-[280px]">
          <div className="flex gap-1 border-b border-slate-100 p-2">
            {leftTabBtn("flows", "Flows", <Bot className="h-4 w-4" />)}
          ...
          <div className="flex gap-2 border-b border-slate-100 px-3 py-2">
            ...
            {vertical === "education" ? (
              <button
                type="button"
                onClick={createEducationEnquiryTemplate}
                className="inline-flex flex-[1.6] items-center justify-center gap-1.5 rounded-lg border border-amber-200 bg-amber-50 px-2 py-2 text-xs font-medium text-amber-800 hover:bg-amber-100"
              >
            ...
            {vertical === "healthcare" ? (
              <div className="flex flex-col gap-2 w-full mt-2">
                ...
                <button
                  type="button"
                  onClick={createHealthcareRegistrationTemplate}
                  className="inline-flex items-center justify-center gap-1.5 rounded-lg border border-teal-200 bg-teal-50 px-2 py-2 text-xs font-medium text-teal-800 hover:bg-teal-100"
                >
                  <Plus className="h-3.5 w-3.5" />
                  Registration
                </button>
                <button
                  type="button"
                  onClick={createHealthcareSlotCheckerTemplate}
                  className="inline-flex items-center justify-center gap-1.5 rounded-lg border border-teal-200 bg-teal-50 px-2 py-2 text-xs font-medium text-teal-800 hover:bg-teal-100"
                >
                ...
                <button
                  type="button"
                  onClick={createHealthcareBookingTemplate}
                  className="inline-flex items-center justify-center gap-1.5 rounded-lg border border-teal-200 bg-teal-50 px-2 py-2 text-xs font-medium text-teal-800 hover:bg-teal-100"
                >
        ...
        {/* Canvas Area */}
        <div className="relative flex min-h-[520px] flex-1 flex-col overflow-hidden xl:min-h-0">
          <ReactFlow
            nodes={nodes}
            edges={edges}
            nodeTypes={nodeTypes}
          ...
          <AnimatePresence>
            {selectedNode && (
              <motion.div
                ...
                className="absolute bottom-0 right-0 top-0 z-10 flex w-full flex-col border-l border-slate-200 bg-white/95 shadow-2xl backdrop-blur sm:w-[380px]"
              >
                <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4">
                  <div>
                    <h2 className="text-base font-semibold text-slate-900">Node Inspector</h2>
                    <p className="mt-0.5 text-xs text-slate-500">Edit properties</p>
                  </div>
                  <button
                ...
                {selectedNode.type !== "start" ? (
                  <div>
                    ...
                    <textarea value={selectedNode.data.text || ""} onChange={(e) => updateSelectedNode({ text: e.target.value })} className="h-28 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm" placeholder="Type node message. Example: Great, you selected {{course}}." />
                    {savedVariables.length > 0 ? (
                      <div className="mt-2 flex flex-wrap gap-1.5">
                        {savedVariables.slice(0, 8).map((variable) => (
                          <button
                            key={variable}
                            type="button"
                            onClick={() => updateSelectedNode({ text: `${selectedNode.data.text || ""}{{${variable}}}` })}
                ...
                {selectedNode.type === "buttons" ? (
                  <div className="space-y-3">
                    <div>
                      <label className="mb-1 block text-xs font-medium text-slate-600">Save Selected Button As</label>
                      <input
                        value={selectedNode.data.saveAs || ""}
                        onChange={(e) => updateSelectedNode({ saveAs: normalizeVariableName(e.target.value) })}
                        className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm"
                        placeholder={inferVariableName(selectedNode.data.text, "selection")}
                      />
                    </div>
                ...
                {selectedNode.type === "list" ? (
                  <div className="space-y-3">
                    <div>
                      <label className="mb-1 block text-xs font-medium text-slate-600">Save Selected Item As</label>
                      <input
                        value={selectedNode.data.saveAs || ""}
                        onChange={(e) => updateSelectedNode({ saveAs: normalizeVariableName(e.target.value) })}
                        className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm"
                    ...
                    <div>
                      <label className="mb-1 block text-xs font-medium text-slate-600">Dynamic Options</label>
                      <select
                        value={selectedNode.data.dynamicList || ""}
                        onChange={(e) => updateSelectedNode({ dynamicList: e.target.value || undefined })}
                        className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm"
                      >
                        <option value="">Manual list items</option>
                        <option value="education_courses">Active education courses</option>
                        <option value="education_batches">Active education batches</option>
                        <option value="healthcare_doctors">Healthcare doctors</option>
                        <option value="healthcare_slots">Healthcare appointment slots</option>
                      </select>
                      {selectedNode.data.dynamicList === "education_courses" ? (
                        <p className="mt-1.5 text-xs text-slate-500">Only courses marked Active on the Courses page appear to students. Save as <span className="font-semibold">course</span> to use <span className="font-semibold">{'{{course}}'}</span> and <span className="font-semibold">{'{{course__id}}'}</span> later.</p>
                      ) : null}
                    </div>
                ...
                <div className="rounded-xl border border-blue-200 bg-blue-50 p-3 text-xs text-blue-800">
                  ...
                  <ul className="space-y-1 pl-4 list-disc">
                    ...
                    <li>Questions, buttons, and lists save variables that can be reused as {"{{"}variable{"}}"} in later messages.</li>
                  </ul>
                  </div>
                </div>
              </div>
