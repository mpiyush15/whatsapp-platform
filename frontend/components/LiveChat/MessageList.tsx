...
export default function MessageList({ messages, socket, isTyping, conversationId, onLoadMore, loadingMore, hasMore, layoutKey }: Props) {
  ...
  const getProxiedMediaUrl = (mediaUrl: string | undefined): string | undefined => {
    ...
    // If it's a WhatsApp URL or S3 URL, proxy through our media endpoint
    if (mediaUrl.includes('graph.facebook.com') || mediaUrl.includes('scontent') || mediaUrl.includes('lookaside.fbsbx.com') || mediaUrl.includes('amazonaws.com')) {
      const encodedUrl = encodeURIComponent(mediaUrl)
      const token = typeof window !== 'undefined' ? localStorage.getItem('token') || localStorage.getItem('authToken') || '' : '';
      return `${process.env.NEXT_PUBLIC_API_URL}/media/proxy?url=${encodedUrl}${token ? `&auth_token=${token}` : ''}`
    }
                      ...
                      <div className="flex flex-col gap-1.5 md:gap-2">
                        ...
                        {(message.mediaUrl?.includes('.pdf') || message.mediaUrl?.startsWith('data:application/pdf')) && (
                          <div className="w-full rounded-lg overflow-hidden bg-gray-100 border border-gray-200">
                            <iframe
                              src={getProxiedMediaUrl(message.mediaUrl)}
                              className="w-full h-40 md:h-64 rounded-lg pointer-events-auto"
                              title="PDF Preview"
                              onLoad={handleMediaLoad}
                              onError={(e) => {
