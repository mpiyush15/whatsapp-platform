...
import { Send, Loader, X, MessageSquare, Paperclip, Settings, Edit2, Save, Check, CheckCheck, Download, MoreVertical, Smile, Play, FileText, Music, Phone, Eye, Copy, Reply, Trash2, Star, Forward, MapPin } from 'lucide-react';
import axios from 'axios';
import io, { Socket } from 'socket.io-client';
import { ConversationStatus, MessageStatus, MessageType, MessageDirection, AssignmentMode, AssignmentStatus } from '@/lib/enums';
...
interface Message {
  ...
  emojiReaction?: string; // For reaction messages
...
export default function LiveChat() {
  ...
  const [conversationStatus, setConversationStatus] = useState<ConversationStatus>(ConversationStatus.OPEN);
  ...
  // ✅ WHATSAPP FEATURE: Send typing indicator with debouncing
  const sendTypingIndicator = useCallback((conversationId: string) => {
    ...
    // Reset timeout (stop typing after 2 seconds of inactivity)
...
// Message events - SINGLE UNIFIED HANDLER (prevents duplicates)
  const handleNewMessage = (data: any) => {
    ...
    // ✅ DEDUPLICATION FIX: Only register ONCE per event
    // Backend emits: 'new_message' (consolidated from old message.sent, message.received, message_received)
    // Cleanup old listeners to prevent handler stacking
    ...
    // ✅ WHATSAPP FEATURES: Typing indicator
    newSocket.on('typing_indicator', (data: any) => {
      console.log('✏️ User typing in conversation:', data.conversationId);
    ...
    // ✅ WHATSAPP FEATURES: Read receipts
    newSocket.on('message.read', (data: any) => {
      console.log('👀 Message read:', data.messageId);
      setMessages(prev => prev.map(msg => 
        msg._id === data.messageId ? { ...msg, status: MessageStatus.READ } : msg
      ));
    });

    // ✅ WHATSAPP FEATURES: Online status
    newSocket.on('user_online', (data: any) => {
      console.log('🟢 Contact online:', data.conversationId);
      setConversations(prev => prev.map(conv => 
    ...
    newSocket.on('user_offline', (data: any) => {
      console.log('⚫ Contact offline:', data.conversationId);
      setConversations(prev => prev.map(conv => 
        conv._id === data.conversationId ? { ...conv, isOnline: false } : conv
      ));
    });
    ...
    <div className="flex h-[calc(100vh-80px)] bg-gray-100">
      ...
      <div className={`w-full md:w-96 bg-white border-r border-gray-200 overflow-y-auto flex flex-col ${selectedConversation ? 'hidden md:flex' : 'flex'}`}>
        ...
        <div className="p-4 border-b border-gray-200 space-y-3">
          ...
          <div className="flex gap-2">
            {(['all', ConversationStatus.OPEN, ConversationStatus.CLOSED] as const).map(status => (
              <button
                key={status}
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {conv.isTyping ? (
                      <p className="text-xs text-green-600 truncate mt-1 font-medium">✏️ typing...</p>
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className={`w-2.5 h-2.5 rounded-full flex-shrink-0 ${
                    conversations.find(c => c._id === selectedConversation)?.status === ConversationStatus.OPEN 
                      ? 'bg-green-500' 
                      : 'bg-gray-400'
              ...
              messages.map((msg, idx) => (
                ...
                >
                  ...
                  <div className={`max-w-md rounded-2xl shadow-sm overflow-hidden relative ${msg.direction === 'outbound' ? 'bg-green-200 rounded-br-none' : 'bg-blue-100 rounded-bl-none'}`}>
                    ...
                    ) : msg.messageType === MessageType.VIDEO && msg.mediaUrl ? (
                      <div className="relative">
                        <img 
                    ...
                    ) : msg.messageType === MessageType.STICKER && msg.stickerUrl ? (
                    ...
                    ) : msg.messageType === MessageType.LOCATION && msg.latitude && msg.longitude ? (
                      <div className="px-4 py-3 bg-inherit">
                        <div className="flex items-start gap-3 bg-white/30 p-3 rounded-lg">
                          <MapPin size={20} className="flex-shrink-0 text-red-600 mt-0.5" />
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium text-gray-900">{msg.address || 'Location'}</p>
                            <p className="text-xs text-gray-600 break-words">{msg.latitude.toFixed(4)}, {msg.longitude.toFixed(4)}</p>
                            <a 
                    ...
                    ) : msg.messageType === MessageType.BUTTON ? (
                      <div className="px-4 py-3 bg-inherit">
                        <p className="text-sm text-gray-900 mb-2">{typeof msg.content === 'string' ? msg.content : 'Button message'}</p>
                        {msg.content && typeof msg.content === 'object' && Array.isArray(msg.content.buttons) && (
                          <div className="flex flex-col gap-2">
                            {msg.content.buttons.map((btn: any, idx: number) => (
                              <button
                                key={idx}
                                onClick={() => console.log('Button clicked:', btn)}
                                className="px-3 py-2 bg-blue-500 text-white text-xs rounded font-medium hover:bg-blue-600 transition text-center w-full"
                              >
                    ...
                    ) : msg.messageType === MessageType.REACTION && msg.emojiReaction ? (
                      <div className="px-3 py-2 bg-inherit">
                        <p className="text-3xl">{msg.emojiReaction}</p>
                      </div>
                    ...
                    ) : msg.messageType === MessageType.DOCUMENT && msg.mediaUrl ? (
                      <div className="px-4 py-3 bg-inherit">
                        <div className="flex items-start gap-3 bg-white/30 p-3 rounded-lg">
                    ...
                    ) : msg.messageType === MessageType.TEMPLATE ? (
                      <div className="px-3 py-2 bg-inherit">
                        <p className="text-xs font-semibold text-gray-700 uppercase tracking-wide mb-1">Template</p>
                        <p className="text-sm break-words text-gray-900">
                          {typeof msg.content === 'object' && msg.content?.templateName
                            ? `${msg.content.templateName}`
                            : typeof msg.content === 'string' ? msg.content : 'Template message'}
                        </p>
                      </div>
                    ) : msg.messageType === MessageType.INTERACTIVE ? (
                      <div className="px-3 py-2 bg-inherit">
                        <p className="text-xs font-semibold text-gray-700 uppercase tracking-wide mb-1">Interactive</p>
                        <p className="text-sm break-words text-gray-900">
                          {typeof msg.content === 'string' ? msg.content : 'Interactive message'}
                        </p>
                      </div>
                    ) : (
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setConversationStatus(e.target.value as ConversationStatus)}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm mt-1 focus:outline-none focus:ring-2 focus:ring-green-500"
                >
      ...
      {mediaViewerOpen && selectedMedia && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
          ...
          {selectedMedia.messageType === MessageType.IMAGE && selectedMedia.mediaUrl && (
          ...
          {selectedMedia.messageType === MessageType.VIDEO && selectedMedia.mediaUrl && (
            <video
              src={selectedMedia.mediaUrl}
              controls
              autoPlay
          ...
          {selectedMedia.messageType === MessageType.STICKER && selectedMedia.stickerUrl && (
            <div className="max-w-2xl w-full">
              <img
                src={selectedMedia.stickerUrl}
                alt="Sticker"
                className="w-full h-auto max-h-96 object-contain rounded-lg"
              />
            </div>
          ...
          {selectedMedia.messageType === MessageType.LOCATION && selectedMedia.latitude && selectedMedia.longitude && (
            <div className="max-w-4xl w-full">
              <iframe
                width="100%"
                height="400"
                frameBorder="0"
                src={`https://www.openstreetmap.org/export/embed.html?bbox=${selectedMedia.longitude - 0.01},${selectedMedia.latitude - 0.01},${selectedMedia.longitude + 0.01},${selectedMedia.latitude + 0.01}&layer=mapnik&marker=${selectedMedia.latitude},${selectedMedia.longitude}`}
                style={{borderRadius: '0.5rem'}}
              ></iframe>
              {selectedMedia.address && (
                <p className="mt-4 text-white text-center">{selectedMedia.address}</p>
              )}
            </div>
          )}
        </div>
