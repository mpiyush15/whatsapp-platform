...
export default function DashboardHealthcareRedirectPage() {
    ...
    const loadProjects = async () => {
      try {
        ...
        if (list.length === 1) {
          ...
          router.replace(`/projects/${targetId}/healthcare`)
        }
      } catch (err) {
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-slate-50">
        <div className="text-center">
          ...
          <p className="mt-3 text-sm text-slate-600">Loading projects for Healthcare...</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-6">
      <div className="mx-auto max-w-4xl rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <h1 className="text-2xl font-bold text-slate-900">Choose a project for Healthcare</h1>
        <p className="mt-1 text-sm text-slate-600">Healthcare modules are project-scoped. Select where you want to work.</p>
        ...
        <div className="mt-6 grid gap-3 md:grid-cols-2">
          {projects.map((project) => {
            ...
            return (
              <Link
                key={targetId}
                href={`/projects/${targetId}/healthcare`}
