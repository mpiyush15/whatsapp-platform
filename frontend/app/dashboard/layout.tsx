...
import { 
  ...
  Activity, DollarSign, Sliders, CreditCard, Receipt, BookOpen, ShieldAlert, Clock, Rocket
} from "lucide-react"
...
function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (!user || user.role !== UserRole.AGENT || !user.agentProjectId) return
    router.replace(`/projects/${user.agentProjectId}/live-chat-v2`)
  }, [user, router])
  ...
  useEffect(() => {
    ...
    // Track user activity (throttled to prevent UI lag)
    let lastActivityUpdate = 0;
    const handleActivity = () => {
      const now = Date.now();
      // Only run every 5 seconds maximum
  ...
  const isSupportTier = pathname.startsWith('/dashboard/support')
  const isPixelsSalon = pathname.startsWith('/dashboard/pixels')
  ...
  if (isPixelsSalon) dashboardHref = '/dashboard/pixels'
    ...
    navigationSections = [
      {
        ...
        items: [
          { name: "Home", icon: LayoutDashboard, href: "/dashboard/pixels", roles: [UserRole.SUPERADMIN, UserRole.ADMIN, UserRole.MANAGER, UserRole.AGENT, UserRole.USER] },
          { name: "Appointments", icon: Clock, href: "/dashboard/pixels/appointments", roles: [UserRole.SUPERADMIN, UserRole.ADMIN, UserRole.MANAGER, UserRole.AGENT, UserRole.USER] },
          { name: "Calendar", icon: Calendar, href: "/dashboard/pixels/calendar", roles: [UserRole.SUPERADMIN, UserRole.ADMIN, UserRole.MANAGER, UserRole.AGENT, UserRole.USER] },
          { name: "Clients", icon: Users, href: "/dashboard/pixels/clients", roles: [UserRole.SUPERADMIN, UserRole.ADMIN, UserRole.MANAGER, UserRole.AGENT, UserRole.USER] },
        ]
      },
      {
        title: 'Management',
        items: [
          { name: "Staff & Services", icon: Users, href: "/dashboard/pixels/staff", roles: [UserRole.SUPERADMIN, UserRole.ADMIN, UserRole.MANAGER] },
          { name: "Memberships", icon: CreditCard, href: "/dashboard/pixels/memberships", roles: [UserRole.SUPERADMIN, UserRole.ADMIN, UserRole.MANAGER] },
        ]
      },
      {
        title: 'Marketing',
        items: [
          { name: "Offers", icon: Megaphone, href: "/dashboard/pixels/offers", roles: [UserRole.SUPERADMIN, UserRole.ADMIN, UserRole.MANAGER] },
          { name: "Loyalty Points", icon: BarChart3, href: "/dashboard/pixels/loyalty", roles: [UserRole.SUPERADMIN, UserRole.ADMIN, UserRole.MANAGER] },
          { name: "AI Chatbot", icon: Bot, href: "/dashboard/features/chatbot", roles: [UserRole.SUPERADMIN, UserRole.ADMIN, UserRole.MANAGER] },
          { name: "WhatsApp Campaigns", icon: Calendar, href: "/dashboard/features/campaigns", roles: [UserRole.SUPERADMIN, UserRole.ADMIN, UserRole.MANAGER] },
          { name: "Templates", icon: FileText, href: "/dashboard/features/templates", roles: [UserRole.SUPERADMIN, UserRole.ADMIN, UserRole.MANAGER] },
        ]
      }
    ]
  } else if (isSuperAdminTier) {
    navigationSections = [
      {
        title: 'Overview',
        items: [
          { name: "Dashboard", icon: LayoutDashboard, href: "/dashboard/superadmin", roles: [UserRole.SUPERADMIN] },
          { name: "Tasks & Calendar", icon: Calendar, href: "/dashboard/superadmin/tasks", roles: [UserRole.SUPERADMIN] },
      ...
      },
      {
        title: 'Billing',
        items: [
          { name: "Financials", icon: DollarSign, href: "/dashboard/superadmin/accounts", roles: [UserRole.SUPERADMIN] },
      ...
      },
      {
        ...
        items: [
          ...
          { name: "Platform contacts", icon: Users, href: "/dashboard/superadmin/contacts", roles: [UserRole.SUPERADMIN] },
        ],
      },
      {
