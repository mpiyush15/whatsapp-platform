          ...
          <div className="space-y-3">
            {payments.map((payment, idx) => (
              <div key={idx} className="bg-gray-50 p-4 rounded-lg border border-gray-200">
                ...
                <p><strong>Status:</strong> {typeof payment.status === 'string' ? payment.status : JSON.stringify(payment.status)}</p>
                <p><strong>Payment Method:</strong> {typeof payment.paymentMethod === 'string' ? payment.paymentMethod : (payment.paymentMethod?.type || JSON.stringify(payment.paymentMethod))}</p>
              ...
              <div key={idx} className="bg-white p-4 rounded-lg border-2 border-blue-300 shadow-sm">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p className="font-semibold text-gray-800">{typeof txn.paymentMethod === 'string' ? txn.paymentMethod : (txn.paymentMethod?.type || JSON.stringify(txn.paymentMethod))}</p>
                  </div>
                  <div>
