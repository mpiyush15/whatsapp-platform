...
type EducationEnquiry = {
  ...
  metadata?: {
    ...
    mappedResponses?: Record<string, unknown>;
...
const mappedResponseKeys = new Set([
  'name', 'fullName', 'full_name', 'customerName', 'customer_name', 'student_name', 'studentName', 'student',
  'email', 'emailAddress', 'email_address',
  'phone', 'mobile', 'mobileNumber', 'mobile_number', 'whatsapp', 'whatsappNumber', 'whatsapp_number',
  'course', 'courseName', 'course_name', 'selectedCourse', 'selected_course',
  'course__id', 'courseId__id', 'course_id', 'courseId',
  'batch', 'batchName', 'batch_name', 'selectedBatch', 'selected_batch',
  'batch__id', 'batchId__id', 'batch_id', 'batchId',
  'fees', 'fee', 'amount', 'courseFees', 'course_fees',
  'notes', 'note', 'remarks', 'remark',
]);

const responseEntries = (enquiry: EducationEnquiry) =>
  Object.entries(enquiry.metadata?.unmappedResponses || enquiry.metadata?.responses || {})
    .filter(([key, value]) => !mappedResponseKeys.has(key) && value !== undefined && value !== null && String(value).trim());

const formatResponseKey = (key: string) =>
  key.replace(/__id$/, ' id').replace(/[_-]+/g, ' ').replace(/\b\w/g, (char) => char.toUpperCase());
  ...
  const visibleEnquiries = useMemo(() => enquiries.filter((enquiry) => {
    ...
    if (term) {
      const haystack = [
        ...
        enquiry.source,
        enquiry.notes,
        ...responseEntries(enquiry).map(([key, value]) => `${key} ${String(value)}`),
          ...
          {initialLoading ? (
            <div className="overflow-x-auto">
              <table className="min-w-[1760px] w-full">
                <thead className="border-b border-slate-200 bg-slate-50">
                  <tr><Th>Student</Th><Th>Course</Th><Th>Batch</Th><Th>Tags</Th><Th>Notes</Th><Th>Responses</Th><Th>Fees</Th><Th>Status</Th><Th>Date</Th><Th>Actions</Th></tr>
                </thead>
                <tbody className="divide-y divide-slate-200"><TableRowsSkeleton rows={7} columns={10} /></tbody>
              </table>
            </div>
            ...
            <div className="animate-content-in overflow-x-auto">
              <table className="min-w-[1900px] w-full">
                <thead className="border-b border-slate-200 bg-slate-50">
                  <tr>
                    <Th>Student</Th><Th>Course</Th><Th>Batch</Th><Th>Tags</Th><Th>Notes</Th><Th>Responses</Th><Th>Fees</Th><Th>Status</Th><Th>Date</Th><Th>Actions</Th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-200">
                  {visibleEnquiries.map((enquiry) => {
                    ...
                    const responses = responseEntries(enquiry);
                    return (
                      <tr key={enquiry._id} className="transition hover:bg-slate-50">
                        ...
                        <Td><div className="max-w-[320px] space-y-1">{responses.length ? responses.slice(0, 4).map(([key, value]) => <p key={key} className="truncate text-xs text-slate-600"><span className="font-medium text-slate-800">{formatResponseKey(key)}:</span> {String(value)}</p>) : <p className="text-xs text-slate-400">No chatbot responses</p>}{responses.length > 4 ? <p className="text-xs text-slate-400">+{responses.length - 4} more</p> : null}</div></Td>
