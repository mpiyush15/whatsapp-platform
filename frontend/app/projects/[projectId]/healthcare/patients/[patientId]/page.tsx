  ...
  return (
    <>
      <motion.div 
        ...
        className="min-h-full w-full pb-16 pt-1 text-slate-800"
      >
        ...
        {loading && !patient ? (
          ...
          <>
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {visibleTabIds.includes("messages") ? (
                    <button
                      type="button"
                      onClick={() => {
                        const phone = patient?.whatsappNumber || patient?.phoneNumber || ""
                        router.push(`/projects/${projectId}/live-chat-v2?phone=${encodeURIComponent(phone)}`)
                      }}
            ...
            <div className="space-y-6">
              ...
              {activeTab === "today" ? (
                <motion.div key="today" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }} className="space-y-4">
            ...
            {activeTab === "visits" ? (
            <motion.section key="visits" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }} className="hc-patient-card hc-tab-panel border-blue-500 space-y-3 p-4">
              ...
              <DataTable
                columns={[
                  ...
                  },
                  {
                    key: "doctor",
                    label: "Doctor",
                    render: (_: any, row: AppointmentRecord) => row.doctorSnapshot?.fullName || row.doctorId || "—",
                  },
                  {
                    key: "status",
                    label: "Status",
                    render: (value: string) => <span className="capitalize">{value || "scheduled"}</span>,
                  },
                  {
                    key: "reason",
                    label: "Reason",
                ...
                loading={appointmentsLoading}
                emptyMessage="No visits yet. Tap Book next visit."
                actions={[
                  {
                    label: "Finish visit",
                    icon: <Receipt className="h-3 w-3" />,
                    onClick: (row) => requestFinishVisit(row as AppointmentRecord),
                    render: (row) =>
                      (row.status !== "completed" &&
                        row.status !== "cancelled" &&
                        row.status !== "no-show" &&
                        row.status !== "checked-in")
                        ? null // Only show if not completed/cancelled/no-show/checked-in
                        : null,
                    disabled: (row) => workflowBusyId === row.appointmentId,
                  },
                ]}
                rowClassName="cursor-pointer hover:bg-blue-50/40"
              />
            </motion.section>
            ) : null}

            {activeTab === "prescription" && showWritePrescription ? (
            <motion.section key="prescription" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }} className="hc-patient-card hc-tab-panel border-violet-500 space-y-3 p-4">
              <div className="flex flex-wrap items-end justify-between gap-2">
                <div>
              ...
              <motion.p key="no-prescription" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }} className="text-sm text-slate-500">Prescription access is not enabled for your login.</motion.p>
            ) : null}

            {activeTab === "followup" ? (
            <motion.section key="followup" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }} className="hc-patient-card hc-tab-panel border-amber-500 space-y-3 p-4">
              <div className="flex flex-wrap items-end justify-between gap-2">
            ...
            {activeTab === "billing" && billingEnabled ? (
              <motion.section key="billing" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }} className="hc-patient-card hc-tab-panel border-rose-500 space-y-3 p-4">
                  ...
                  <DataTable
                    columns={[
                      ...
                      },
                      {
                        ...
                        label: "Due",
                        render: (value: number) =>
                          <span className="text-rose-600">₹{Number(value || 0).toLocaleString("en-IN")}</span>,
                      },
                      {
                        key: "status",
                        label: "Status",
                        render: (value: string) => <span className="capitalize">{value || "draft"}</span>,
                      },
                    ]}
            ...
            {activeTab === "messages" ? (
              <motion.section key="messages" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }} className="hc-patient-card hc-tab-panel border-green-500 space-y-3 p-4">
            ...
            {activeTab === "details" ? (
            <motion.section key="details" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }} className="hc-patient-card hc-tab-panel border-slate-400 space-y-3 p-4">
      ...
      {completeVisitModal ? (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4">
          <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-5 shadow-2xl">
      ...
      {showEditModal ? (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4">
          <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="max-h-[90vh] w-full max-w-5xl overflow-y-auto rounded-2xl border border-slate-200 bg-white p-5 shadow-2xl">
      ...
      {showFollowUpModal ? (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4">
          <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl border border-slate-200 bg-white p-5 shadow-2xl">
            <div className="mb-4 flex items-start justify-between gap-4">
              <div>
