...
export default function MedicineMasterPage() {
  ...
  const handleStatusChange = useCallback(async (productId: string, newStatus: string) => {
    try {
      setProducts(current => current.map(p => p.productId === productId ? { ...p, status: newStatus } : p))
      await apiPut(`/healthcare/pharmacy-products/${productId}`, { projectId, status: newStatus })
  ...
  const productColumns = useMemo(
    () => [
      ...
      { key: "category", label: "Category", render: (v: string) => v || "—" },
      {
        key: "status",
        label: "Status",
        render: (v: string, row: PharmacyProductRecord) => {
          const currentStatus = v || "active"
          return (
            <select
              value={currentStatus}
              onChange={(e) => handleStatusChange(row.productId, e.target.value)}
              className={`rounded-full px-2.5 py-1.5 text-xs font-semibold uppercase outline-none cursor-pointer border transition-colors ${
  ...
  const handleSubmitProduct = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (editingProductId) {
        const payload = await apiPut<{ data?: { product: PharmacyProductRecord } }>(`/healthcare/pharmacy-products/${editingProductId}`, payloadBody)
        if (payload?.data?.product) {
          setProducts((current) => current.map(p => p.productId === editingProductId ? payload.data!.product : p))
        }
      ...
      setError(err instanceof Error ? err.message : `Failed to ${editingProductId ? "update" : "add"} medicine`)
    } finally {
      setSubmitting(false)
    }
  }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="flex max-h-[90vh] w-full max-w-lg flex-col rounded-2xl border border-slate-200 bg-white shadow-2xl">
            <div className="flex items-start justify-between border-b border-slate-100 p-5">
              <div>
                <h2 className="text-lg font-semibold text-slate-900">{editingProductId ? "Edit medicine" : "Add medicine"}</h2>
            ...
            <form id="create-medicine-form" onSubmit={handleSubmitProduct} className="flex min-h-0 flex-1 flex-col">
              ...
              <div className="flex gap-3 border-t border-slate-100 p-4">
                ...
                >
                  {submitting ? <Loader2 className="h-4 w-4 animate-spin" /> : editingProductId ? null : <Plus className="h-4 w-4" />}
                  {submitting ? "Saving…" : editingProductId ? "Update medicine" : "Save medicine"}
                </button>
              </div>
            </form>
          </div>
