...
type RecipientFilter = 'all' | 'opened' | 'replied' | 'not_replied' | 'failed';
...
export default function CampaignDetailPage() {
  ...
  const audienceCounts = useMemo(() => {
    ...
    const failed = recipients.filter((r) => r.outboundStatus === 'failed').length;
    return { total: recipients.length, opened, repliers, failed };
  }, [recipients]);
  ...
  const filteredRecipients = useMemo(() => {
    switch (recipientFilter) {
      ...
      case 'not_replied':
        return recipients.filter((r) => !r.replied);
      case 'failed':
        return recipients.filter((r) => r.outboundStatus === 'failed');
      default:
        return recipients;
    }
  ...
  const exportCsv = useCallback(() => {
    ...
    const header = ['Name', 'Phone', 'Status', 'Replied', 'Reply Count'].join(',');
    const rows = filteredRecipients.map(r => 
      [
        `"${r.name || ''}"`,
        `"${formatPhone(r.phone)}"`,
        `"${r.outboundStatus || 'pending'}"`,
        `"${r.replied ? 'Yes' : 'No'}"`,
        r.replyCount || 0
      ].join(',')
    );
    ...
    link.href = url;
    link.download = `campaign-${campaign?.name || campaignId}-${recipientFilter}-recipients.csv`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  }, [filteredRecipients, campaign, campaignId, recipientFilter]);
        ...
        <div className="px-4 py-3 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">
              ...
              {audienceCounts.repliers} replied · {audienceCounts.failed} failed
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            <button
              type="button"
              onClick={exportCsv}
              className="px-3 py-1.5 rounded-lg text-xs font-medium bg-white border border-gray-200 text-gray-700 hover:bg-gray-50 flex items-center gap-1.5 transition"
            >
