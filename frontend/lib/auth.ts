    ...
    try {
      ...
      if (response.ok && data.success && token) {
        const user: User = {
          ...
          userId: data.data.user.userId || null,
          ...
          agentId: data.data.user.agentId || null,
          agentMongoId: data.data.user.agentMongoId || null,
          agentRole: data.data.user.agentRole || null,
          agentProjectId: data.data.user.agentProjectId || null,
...
export const getPostLoginRedirect = (user: User | null): string => {
  ...
  if (user.role === UserRole.AGENT && user.agentProjectId) {
    return `/projects/${user.agentProjectId}/live-chat-v2`
  }
