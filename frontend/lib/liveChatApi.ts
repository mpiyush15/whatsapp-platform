...
export type LiveChatAgent = {
  ...
  userId?: string | { _id: string; email?: string; phone?: string };
  availability?: {
...
export async function fetchLiveChatAgents(projectId?: string): Promise<LiveChatAgent[]> {
  const q = new URLSearchParams();
  q.set('status', 'active');
  if (projectId) q.set('projectId', projectId);

  const res = await fetch(`${API_URL}/agents?${q}`, { headers: authHeaders(false) });
  if (!res.ok) throw new Error('Failed to load agents');
  const payload = await res.json();
  return payload.agents || [];
}

export async function assignConversation(conversationId: string, agentId?: string, projectId?: string) {
  const res = await fetch(`${CONV_BASE}/${conversationId}/assign`, {
    method: 'POST',
    headers: authHeaders(),
    body: JSON.stringify({ ...(agentId ? { agentId } : {}), ...(projectId ? { projectId } : {}) }),
  });
...
export async function saveConversationAsLead(input: {
  conversationId: string;
  projectId?: string;
  name: string;
  phone: string;
  ...
  const q = input.projectId ? `?projectId=${encodeURIComponent(input.projectId)}` : '';
  const res = await fetch(`${API_URL}/leads${q}`, {
    method: 'POST',
    headers: authHeaders(),
    body: JSON.stringify({
  ...
  return payload.data || payload.lead;
}
